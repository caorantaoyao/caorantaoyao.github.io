...
<title>{title} — CAORAN</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Noto+Serif+SC:wght@400;500;600;700&family=JetBrains+Mono:wght@300;400;500&display=swap" rel="stylesheet">
<style>
  :root {{
    --bg:#0a0a0c; --surface:#121216; --surface-raised:#17171c; --border:#232327; --border-soft:#1b1b1f;
    --text:#e6e2da; --text-muted:#837e76; --text-dim:#4c4842;
    --accent:#c9a87c; --accent-bright:#dcc199; --accent-glow:rgba(201,168,124,0.14); --accent-dim:#8a6d4b;
    --green:#86a596; --green-dim:#4a6a5a; --red:#c98a8a; --blue:#7f96b0;
  ...
  *,*::before,*::after {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  ::selection {{ background:var(--accent-dim); color:#0a0a0c; }}
  body::after {{ content:''; position:fixed; inset:0; z-index:0; pointer-events:none;
    background:radial-gradient(ellipse 70% 45% at 50% -10%, var(--accent-glow) 0%, transparent 60%); }}
  ...
  nav {{ position:sticky; top:0; z-index:100; padding:1.2rem 0;
    ...
    background:rgba(10,10,12,0.75); border-bottom:1px solid var(--border-soft); }}
  ...
  .nav-logo .dot {{ width:6px; height:6px; border-radius:50%; background:var(--accent); box-shadow:0 0 10px var(--accent); }}
  ...
  .article-tag.security {{ color:var(--red); border-color:rgba(201,124,124,0.3); }}
  h1.article-title {{ font-size:clamp(1.8rem,4vw,2.6rem); font-weight:700; line-height:1.3;
    letter-spacing:-0.02em; margin-bottom:2.5rem; }}
  .article-body {{ font-size:1.02rem; color:#d4d0c8; }}
  .article-body h2 {{ font-size:1.5rem; font-weight:600; margin:2.5rem 0 1rem;
    color:var(--text); letter-spacing:-0.01em; }}
  .article-body h3 {{ font-size:1.2rem; font-weight:600; margin:2rem 0 0.8rem; color:var(--text); }}
  ...
  .article-body code {{ font-family:var(--font-mono); font-size:0.85em;
    background:var(--surface-raised); padding:0.15em 0.4em; border-radius:3px; color:var(--accent); }}
  ...
  .article-body pre code {{ background:none; padding:0; color:#c8c4bc; font-size:0.82rem; line-height:1.7; }}
...
<footer>&copy; 2026 Caoran · 内容同步自飞书知识库</footer>
<script>
